...
Ubuntu is a very popular Linux distribution built around ease of use. Ubuntu
is based on the Debian project and and has spawned its own family of
distributions. What defines the Debian family of distributions is the use
of the DEB package format and the dpkg/apt package managers.
...
Packaging manually for Debian-based distros such as Ubuntu can frustrating, especially
for first timers. That’s why for this class, we’ll be using a really cool Ruby gem
called `fpm` (Effing Package Manager) which simplifies the task of packaging a lot.
