...
Here is a link to the Google form: <link>
...
but for the rest of this exercise I’m going to assume that you did this. You might see some weird output like 
...
Some [quick definitions from Docker’s 
website:](https://docs.docker.com/get-started/#a-brief-explanation-of-containers)
...
you are curious about other variants of Linux, you can run a lot of them inside containers as well! You can try running 
[Fedora](https://getfedora.org/) (_\*tips hat\*_ M'Linux), another long-running Linux distribution.:
...
should
check out the [reference page](https://docs.docker.com/engine/reference/builder/) if you are trying to do anything more 
complex.
...
executable
(located in `/usr/games/fortune` after you install it). Note that you won’t need to use the `-it` flags when you run 
...
you don’t have anything else running on that port (if you're not on campus, you can just `curl 
<url_of_host_machine>:5050` from your VM or another machine on campus, e.g. ssh.ocf.berkeley.edu), and see the words 
"**It works!**"
...
persistent database available on the market today, [MongoDB](https://www.youtube.com/watch?v=b2F-DItXtZs). _Amazing_, 
you think to yourself, _every word of what he just said was wrong_.

The web application can be found on GitHub. Note that the web app listens on port 8080, so you'll need to forward or 
expose that port. Instructions for setting it up are located in the `README.md` of the repository:
