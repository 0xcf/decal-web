...
1. Download[^wget] the two files named `a8q1.gpg` and `a8q2.gpg` from
   [https://decal.ocf.berkeley.edu/static/a8/](https://decal.ocf.berkeley.edu/static/a8/),
   decrypt them using `gpg` and the passwords `weak_password` and
   `weaker_password` respectively, and answer the questions contained therein.
...
2. Create a text file with your full name, username, and a random question in
   ...
   distribute this encrypted file to someone else in the lab[^nginx], and have
...
3. The DeCal staff have a [public key
   available](http://decal.ocf.berkeley.edu/static/a8/staff_public_key.gpg).
...
5. Create a text file with a new random question, as in task 2. Find someone in
...
6. Repeat step 5, but this time, sign the encrypted file, and verify the
   signature on the file your receive in turn, using the appropriate `gpg`
   options.

7. The DeCal staff have uploaded a
   [file](http://decal.ocf.berkeley.edu/static/a8/ocf_address.asc) to help you
   ...
   location. Figure out if the file has been modified, and if so, fix the file.
