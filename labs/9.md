...
### Enable IPv6 for Docker Daemon (Only Needed if Using Your Decal VM)
1. **Edit the Configuration File**: Use `sudo` to edit the `/etc/docker/daemon.json` file and add the following settings:
    ```json
    {
        "ipv6": true,
    ...
    ```
2. **Create an IPv6 Network**: Execute the following command to set up an IPv6 network:
    ```bash
    sudo docker network create --ipv6 --subnet 2001:0DB8::/112 ip6net
    ```
3. **Restart the Docker Daemon**: To apply the changes, restart the Docker service with the following command:
    ```bash
    sudo systemctl restart docker
    ```

For more details on configuring IPv6, you can consult the [official Docker IPv6 documentation](https://docs.docker.com/config/daemon/ipv6/).
