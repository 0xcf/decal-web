...
[decal-labs repo][decal-labs-7] using `wget` or something similar to download
...
You can test out each worker if you'd like by forwarding each of the ports
...
[lab 4](https://decal.ocf.berkeley.edu/labs/4/).
...
Make sure to forward ports 7000 and 7001 to allow access to your load balancer server and the
stats page from your local machine.

After adding this, if you restart the `haproxy` service and open
`http://localhost:7001/stats` in a web browser, you should see a
...
7000 on your VM, so that if you go to `http://localhost:7000` you
...
If you make a request to `http://localhost:7000/crash`, it will
...
`http://localhost:7000` again, are all the servers present in the
