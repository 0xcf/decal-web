...
### Questions
2. Here's a text file containing all valid Scrabble words: https://raw.githubusercontent.com/raun/Scrabble/master/words.txt. Download this file and write a one-line command to print out the first 20 words that do not contain any of the letters A, E, I, O, U, or Y. Text manipulation commands like `grep`, `awk` or `sed` might be useful!
3. We want to find all Scrabble words that contain 'Cal' in them. We already know that we can do `cat words.txt | grep "CAL"` to do this, but how might you be able to do it without using a pipe? (Hopefully this will help you appreciate pipes just a little bit more.)
...
### Questions
 ...
 ![tmux layout](../images/tmux-layout.png)
