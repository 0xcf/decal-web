...
someone else yourself and do the same.[^extracredit]
...
for you. Decrypt their file and keep it around for checkoff. [^extracredit]
...
1. Download a copy of the CentOS 7 NetInstall ISO from the [OCF mirrors](http://mirrors.ocf.berkeley.edu/centos/7/isos/x86_64/)
and verify that its SHA256 hash is correct.
Also compute its SHA1 and MD5 hashes.

2. You may have noticed that the CentOS project provides a signature over the 
hashes it provides (the .asc file in the same directoy as above).
Briefly explain why this is a good, efficient security 
...
1. Customize the file at `/var/www/html/index.html` to reflect your indiviuality
