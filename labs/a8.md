...
[xor-example]: https://www.pcmag.com/encyclopedia_images/XOR.GIF "xor in crypto"
...
In [lab a4](/a4), one task was to generate an SSH key, using the command
...
Please use this [Google Form](https://docs.google.com/forms/d/e/1FAIpQLSdLVyg8hiEy5oB4HzE9QRSH9dJA4FIWBJYYriSvd2O7yQl7yg/viewform?usp=sf_link)
for submitting solutions for this lab.
...
3. The DeCal staff have a [public key available](http://decal.ocf.berkeley.edu/static/a8/staff_public_key.gpg).
...
[file](http://decal.ocf.berkeley.edu/static/a8/ocf_address.asc) to help you figure out 
how to get to the OCF. However, we suspect nefarious elements may be trying to 
modify the file in an effort to divert students to their location. Figure out if the
file has been modified, and if so, fix the file.
...
1. Download a copy of the CentOS 7 NetInstall ISO from the OCF mirrors 
(http://mirrors.ocf.berkeley.edu) and verify that its SHA256 hash is correct.
Also compute its SHA1 and MD5 hashes.
...
account information stored in a file called `/etc/passwd` and passwords in `/etc/shadow`.
These files are owned by `root`, preventing regular users from reading or changing 
this information without permission.
...
port they are listening on. Use the tools you learned about in lab 5, such as
`netstat`, as well as tools like `ps`. You may also want to point `nmap -A`
at your VM from another machine, such as `tsunami`.
...
[^nginx]: consider that you have a functioning web server (`apache` or `nginx`) running on your student VM already from lab 6. It is advisable to use `nginx`. Furthermore, take a look at the [autoindex](http://nginxlibrary.com/enable-directory-listing/) directive. Another option would be to use your OCF-provided [web hosting](https://www.ocf.berkeley.edu/docs/services/web/), or using netcat to transfer the files.
